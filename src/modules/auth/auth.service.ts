...
import { UserServce } from '../user/user.service';

@Injectable()
export class AuthService {
  constructor(
    private readonly usersService: UserServce,
    private readonly jwtService: JwtService,
    private readonly configService: ConfigService,
  ) {}

  async validateUserByNamePassword(name: string, password: string) {
    const user = await this.usersService.getByName(name).catch(() => {
